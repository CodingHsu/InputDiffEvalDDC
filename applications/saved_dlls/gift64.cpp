...
extern "C" __declspec(dllexport) void encrypt(uint8_t plaintext[8], uint8_t masterkey[16], int rounds, uint8_t ciphertext[8]){
	uint8_t input[16];
	for (int i=0; i<8; i++){
...
extern "C" __declspec(dllexport) void encrypt_batch(uint8_t plaintext[][8], uint8_t masterkey[][16], int rounds, uint8_t ciphertext[][8], const int batch_size) {
