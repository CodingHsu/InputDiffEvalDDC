...
void generateRoundKeys(uint8_t *key, int rounds, uint8_t round_keys[][8]){
...
extern "C" __declspec(dllexport) void encrypt(uint8_t in[8], uint8_t key[10], int rounds, uint8_t out[8]) {
	uint64_t state;
	uint8_array_to_uint64(in, &state);
	uint8_t round_keys[rounds+1][8];
	generateRoundKeys(key, rounds, round_keys);
    ...
    uint64_to_uint8_array(state, out);
}

extern "C" __declspec(dllexport) void encrypt_batch(uint8_t in[][8], uint8_t key[][10], int rounds, uint8_t out[][8], const int batch_size) {
    for (int i = 0; i < batch_size; i++) {
        encrypt(in[i], key[i], rounds, out[i]);
    }
}
