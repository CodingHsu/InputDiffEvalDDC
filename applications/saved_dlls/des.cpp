...
extern "C" __declspec(dllexport) void encrypt(const uint8_t input[8], const uint8_t inKey[8], const int rounds, uint8_t output[8])
{
    int ip[64] = {0};
    int output_1[64] = {0};
    int subKeys[16][48];
...
extern "C" __declspec(dllexport) void encrypt_batch(const uint8_t input[][8], const uint8_t inKey[][8], const int rounds, uint8_t output[][8], const int batch_size){
	for (int i=0; i<batch_size; ++i){
		encrypt(input[i], inKey[i], rounds, output[i]);
	}	
}

extern "C" __declspec(dllexport) void decrypt(const uint8_t input[8], const uint8_t inKey[8], const int rounds, uint8_t output[8])
{
    int ip[64] = {0};
    int output_1[64] = {0};
    int output_2[64] = {0};
    ...
    uint8ToBitArray(input, ip, 8);
//    IP_Init_Rep(ip, output_1, IP_Init_Table);
    uint8ToBitArray(inKey, key, 8);
    subKey_fun(key, subKeys);
    ...
    for (int j = 1; j < rounds; j++)
    {
        ...
        F_func(r[j - 1], r[j], subKeys[rounds - j]);
        Xor(r[j], l[j - 1], 32);
    }
    ...
    F_func(r[rounds-1], l[rounds], subKeys[0]);
    Xor(l[rounds], l[rounds-1], 32);
...
extern "C" __declspec(dllexport) void decrypt_batch(const uint8_t input[][8], const uint8_t inKey[][8], const int rounds, uint8_t output[][8], const int batch_size){
	for (int i=0; i<batch_size; ++i){
		decrypt(input[i], inKey[i], rounds, output[i]);
	}	
}
